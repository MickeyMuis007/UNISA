...
void ProductForm::commit() {
    ...
    qDebug()<<m_model->getName();
    qDebug()<<m_model->getDescription();
    qDebug()<<m_model->getDateAdded();
    qDebug()<<m_model->getPrice();
}

void ProductForm::update() {
    if (m_model ==0) return;
    qDebug() << "update()";
    m_ui->nameLineEdit->setText(m_model->getName());
    m_ui->priceSpinbox->setValue(m_model->getPrice());
    m_ui->dateEdit->setDate(m_model->getDateAdded());
    m_ui->descriptionEdit->setText(m_model->getDescription());
}
